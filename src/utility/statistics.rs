...
/// ```
pub fn median<T>(slice: &mut [T]) -> T 
    where T: Copy + cmp::Ord + num::One + num::Zero + ops::Div<T, Output = T> 
{
    let len = slice.len();
...
/// assert_eq!(median.round(), 4.0);
/// ```
pub fn median_sorted<T>(sorted_slice: &[T]) -> T 
    where T: Copy + num::One + num::Zero + ops::Div<T, Output = T> 
{
